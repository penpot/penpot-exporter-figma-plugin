...
export type Drawable = { pathData: string; transform: Transform };

export const extractDrawablePaths = (svg: string): Drawable[] => {
...
export const computePathBounds = (subpaths: Command[][]): PathBounds | undefined => {
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
  let maxY = -Infinity;
