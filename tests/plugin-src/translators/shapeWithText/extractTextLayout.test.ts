...
import { extractTextLayoutFromDrawables } from '@plugin/translators/shapeWithText/extractTextLayout';

const aabb = { x: 314, y: 460, width: 200, height: 100 };

const textDrawables = (editable: string, outlined: string): Drawable[] => {
  const editableDrawables = extractDrawablePaths(editable);
  const outlinedDrawables = extractDrawablePaths(outlined);
  return outlinedDrawables.slice(editableDrawables.length);
};

describe('extractTextLayoutFromDrawables', () => {
  it('returns undefined when there are no text drawables', () => {
    expect(extractTextLayoutFromDrawables([], aabb)).toBeUndefined();
  });

  it('pads frame width around the glyph bbox center to absorb Penpot metric drift', () => {
    // text glyph at (20..60, 30..50): bbWidth=40, pad=max(40*0.2, 8)=8 → width=48.
  ...
  it('matches the bbox of a real Figma SQUARE export (HEADER 3 fixture)', () => {
    ...
    const layout = extractTextLayoutFromDrawables(textDrawables(editable, outlined), {
      x: 0,
      y: 0,
      width: 176,
      height: 176
